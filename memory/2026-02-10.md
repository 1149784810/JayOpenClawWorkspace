...
### 15. 依赖树更新 - CD System (2026-02-10 20:04)
**触发**: CD机制需求分析结束后立即更新
...
└── CD System (CD冷却系统) [2026-02-10新增]
    ...
    │   ├── Methods: InitializeCardCDs(), ProcessCardCDs(), ExecuteCDEffect()
    │   └── Helper: GetLeftNeighbor(), GetRightNeighbor(), CheckBattleEnd()
    ├── Network Files: MsgCDUpdate, MsgCDEffect
    └── UI Files: ShowCDIndicator(), PlayCDEffectAnimation()
```

**效果类型覆盖**:
1. Accelerate - 加速相邻卡牌CD
2. Slow - 减速敌方卡牌CD
3. Damage - 对敌方造成伤害（触发战斗结束检查）
4. Heal - 对己方回复生命

**Quick Reference**: 添加了完整的CD System实现清单，包含7个步骤的检查点。

**验证**: 依赖树已能覆盖此需求的全部实现路径 ✅

### 16. 依赖树更新 - Card Slot Initialization (2026-02-10 20:15)
**触发**: 回答 card.slot 初始化位置问题后立即更新
...
**关键发现**:
- Card 构造函数不初始化 slot 字段
- 默认值为 Slot.None (x=0)
- 首次有效赋值发生在 PlayCard() 或 MoveCard() 被调用时

**更新位置**: skills/xiuxian-gamedev/SKILL.md - Card Data 分支下

### 17. 推挤系统BUG修复（2026-02-10 22:19）
**问题报告**: 服务器端完成推挤算法后，客户端卡牌未移动到正确位置

**排查过程**:
1. 检查服务器端 OnCardMoved 广播 - 正常
2. 检查客户端 OnMove 处理 - 正常
3. 检查 BSlot.Get(slot) 查找逻辑 - 正常
4. **发现问题**: 服务器端 MoveCard 方法中，推挤循环只更新了 cardToPush.slot，但没有触发 onCardMoved 事件
...
for (int i = cardsToPush.Count - 1; i >= 0; i--)
{
    ...
    cardToPush.slot = new Slot(targetX, slot.type);
    // 缺少: onCardMoved?.Invoke(cardToPush, cardToPush.slot);
}
...
for (int i = cardsToPush.Count - 1; i >= 0; i--)
{
    ...
    // 修复：触发被推挤卡牌的位置更新事件
    onCardMoved?.Invoke(cardToPush, cardToPush.slot);
}
```

**修复文件**: GameLogic.cs (MoveCard 方法中的推挤循环)

**验证**: 现在服务器会为每张被推挤的卡牌触发位置更新事件，客户端能正确接收并更新位置

**更新位置**: skills/xiuxian-gamedev/SKILL.md - Push System 分支下添加BUG记录

### 18. Medium Card Position 实现（2026-02-10 22:36）
**需求**: 中型卡（占据2个槽位）的表现层中心位置应放置在其所占据的两个slot中心

**实现方案**:
```csharp
// BoardCard.CalculateCardPosition()
private Vector3 CalculateCardPosition(Slot slot, Card card)
{
    int cardSize = GetCardSize(card); // 1/2/3
    
    if (cardSize <= 1)
        return mainPos; // Small card: center of single slot
    
    // Multi-slot cards: calculate center between occupied slots
    int lastSlotX = slot.x + (cardSize - 1);
    Slot lastSlot = new Slot(lastSlotX, slot.type);
    Vector3 lastPos = BSlot.Get(lastSlot).GetPosition(lastSlot);
    
    return (mainPos + lastPos) / 2f; // Center position
}
```

**新增方法**:
1. `CalculateCardPosition(Slot slot, Card card)` - 根据卡牌大小计算显示位置
2. `GetCardSize(Card card)` - 获取卡牌占据的槽位数量 (1/2/3)

**修改方法**:
- `OnMove(Card card, Slot slot)` - 使用 CalculateCardPosition 替代直接获取槽位位置
...
**更新位置**: skills/xiuxian-gamedev/SKILL.md - Push System 分支下
...
### 19. Multi-Slot Card System 重大架构变更（2026-02-10 22:48）
**需求**: Card.slot 改为数组形式 Card.slots[]，支持多槽位卡牌

**核心变更**:

**1. Card.cs - 字段与方法**
```csharp
public Slot slot;  // 保持网络兼容
public List<Slot> slots = new List<Slot>();  // 占据的所有槽位

public Slot GetMainSlot() => slots.Count > 0 ? slots[0] : Slot.None;
public bool OccupiesSlot(Slot checkSlot) => slots.Contains(checkSlot);
public void SetSlots(List<Slot> newSlots);  // 同时更新 slot 字段
public int GetCardSize();  // 1/2/3 基于 CardData.size
```

**2. Game.cs - GetSlotCard**
```csharp
// 修改前: if (card.slot == slot)
// 修改后: if (card.OccupiesSlot(slot))
```

**3. GameLogic.cs - MoveCard 完全重写**
- `CalculateTargetSlots()` - 根据卡牌大小计算目标槽位
- `PlaceCardAtSlots()` - 放置卡牌到多个槽位
- `TryPushCards()` - 支持多槽位卡牌的推挤逻辑

**4. BoardCard.cs - 中型卡特殊处理**
```csharp
// CalculateMediumCardTargetSlot() - 中轴线判定
// 鼠标在左中轴线左侧 → 使用 (x-1, x)
// 鼠标在左右中轴线之间 → 使用 (x-1, x)
// 鼠标在右中轴线右侧 → 使用 (x, x+1)
// 边界检查：x=1 或 x=max 时返回 Slot.None（非法）
```

**边界处理**:
- 中型卡靠近最左侧或最右侧中轴线 → 非法，回到原位置
- 大型卡拖动到边界 → 需要左右各一个槽位，否则非法

**网络兼容性**:
- 保持 `slot` 字段用于序列化
- SetSlots() 自动更新 slot = slots[0]
- 所有网络消息继续使用 slot（主槽位）

**修改文件清单**:
- Card.cs, Game.cs, GameLogic.cs, BoardCard.cs

**更新位置**: 
- skills/xiuxian-gamedev/SKILL.md - Changelog v1.1
- skills/xiuxian-gamedev/SKILL.md - Requirement Path Tree 中 Card Data 分支
- skills/xiuxian-gamedev/SKILL.md - Requirement Path Tree 中 Slot System 分支
- skills/xiuxian-gamedev/SKILL.md - Quick Reference 添加 Multi-Slot Card System 完整清单

### 20. 需求依赖树更新（2026-02-10 23:18）
**更新内容**:

**1. Card Data 分支新增 Multi-Slot Card System 章节**:
- Architecture Change: Card.slot → Card.slots[]
- Core Concept: Cards occupy 1-3 slots based on CardSize
- New Methods: GetMainSlot(), OccupiesSlot(), SetSlots(), GetCardSize()
- Access Pattern Rules (MANDATORY): 
  - ❌ NEVER: card.slot == targetSlot
  - ✅ ALWAYS: card.GetMainSlot() == targetSlot
- Files to Update: Card.cs, Game.cs, Player.cs, GameLogic.cs, BoardCard.cs, AILogic.cs, ConditionSelf.cs
- Medium Card Logic: Mouse-axis detection for slot pair selection
- Big Card Logic: Center slot + left/right neighbors
- Network Compatibility: Keep slot field for serialization
- Pre-Flight Checklist: Search ALL references before modifying
- Common Pitfalls: Player.GetSlotCard(), AILogic equipment targeting, etc.

**2. Slot System 分支新增 Multi-Slot Impact**:
- Cards can occupy multiple slots (1-3)
- Slot comparison: Use Card.OccupiesSlot()
- Slot retrieval: Use Card.GetMainSlot()
- Boundary validation required

**3. Quick Reference 新增 Multi-Slot Card System 完整清单**:
- PRE-FLIGHT: Search ALL references
...
### 21. 推挤算法BUG修复（2026-02-10 23:25）
**问题报告**: 多槽位系统重构后，推挤算法无法正常工作

**排查过程**:
1. 检查 TryPushCards() 方法逻辑
2. 发现推动距离计算错误：使用了 cardSize 而不是 1
3. 发现缺少被推动卡牌之间的冲突检测
...
```csharp
// 错误
int newMainX = pushLeft ? mainSlot.x - cardSize : mainSlot.x + cardSize;

// 正确（被推动的卡牌整体移动1格）
int newMainX = pushLeft ? mainSlot.x - 1 : mainSlot.x + 1;
```

**修复2 - 边界检查**:
```csharp
// 根据卡牌大小检查所有槽位是否在边界内
int leftmostX = newMainX;
int rightmostX = newMainX;
if (pushSize == 2) {
    int offset = mainSlot.x - cardToPush.slots[0].x;
    leftmostX = newMainX - offset;
    rightmostX = leftmostX + 1;
} else if (pushSize == 3) {
    leftmostX = newMainX - 1;
    rightmostX = newMainX + 1;
}
// 检查: if (leftmostX < Slot.x_min || rightmostX > Slot.x_max) return false;
```

**修复3 - 冲突检测**:
```csharp
// 验证被推动的卡牌之间不会重叠
HashSet<Slot> occupiedSlots = new HashSet<Slot>();
foreach (var kvp in newPositions) {
    foreach (Slot slot in kvp.Value) {
...
**更新位置**: 
- GameLogic.cs - TryPushCards() 方法
- skills/xiuxian-gamedev/SKILL.md - Push System BUG记录
